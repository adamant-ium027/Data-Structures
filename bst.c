...
bool enqueue(struct Queue *qu, struct Node *node){
    if(qu->rear == qu->length-1 && qu->front == 0) return false;
    if(qu->front == qu->rear+1) return false;
...
// void dfs(struct BSTree *); // height order traversal
...
struct Node *insert(struct BSTree *bst, int data){
    ...
    if(bst->head == NULL) return temp; // if data is to be inserted at root.
...
void preorder(struct BSTree *bst){
    ...
    initStack(&stack1, array, 100);
    push(&stack1, ptr);

    while(isEmptyStack(&stack1) == false){
        ptr = pop(&stack1);
...
int heightStack(struct BSTree *bst){
    ...
    while(curr_node != NULL || !isEmptyStack(&stack1)){
        if(curr_node != NULL){
            ...
            struct Node *peek_node = peek(&stack1);
